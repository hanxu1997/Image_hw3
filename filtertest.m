...
la_result = filter2d_freq( input_img, filter4);
figure(3);
% 输出原图像
subplot(1,3,1);
imshow(input_img);
...
% Laplacian滤波器滤波结果
subplot(1,3,2);
imshow(scaling(uint8(la_result)));
title('Laplacian Result');
imwrite(scaling(uint8(la_result)), '.\Filter_result\Laplacian_filter.png');
% 锐化原图像
output_img4 = scaling(uint8(la_result)) + input_img;
subplot(1,3,3);
% output_img4 = scaling(output_img4);
imwrite(scaling(output_img4), '.\Filter_result\Laplacian_Sharpen.png');
